...
Review these invariants before invoking the CLI:

1. `Platform` is `macos` and `InstallerType` is `pkg`.
2. `AppType` is `pkg` or `lob` (or is omitted only where the target schema explicitly
   defines the default as `pkg`).
3. The entry has one PKG `Source`, required `Requirements`, and `Detection` fields
   required by the target schema.
4. `IncludedApps` has 1–500 entries, no ordinal/case-sensitive duplicate
   `BundleId`, and non-empty `BundleId`/`BundleVersion` values.
5. Each LOB entry has a non-empty `BundleBuildVersion`; no PKG entry generates one.
6. A present `PrimaryBundleId` is non-blank and has exactly one exact or

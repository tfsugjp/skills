...
4. If the eligible target is newer than the pull request, use `scripts/refresh_policy.py` to enforce the bounded state transition. Comment exactly `@dependabot recreate` before any direct edit. Poll every 30 seconds for at most 20 attempts. Re-read the head SHA and diff after every change.
5. If recreate times out or remains stale, check out the same pull-request branch and build argv with `scripts/native_update.py`. Run the ecosystem's native targeted updater with lifecycle scripts disabled where supported. Preserve the manifest declaration style, regenerate only the associated lock or verification files, and reject unexpected changes.
